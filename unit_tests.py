...
class MyTestCase(unittest.TestCase):
    ...
    def test_should_return_NotIntException(self):
        '''sprawdzenie czy jesli ilosc monet nie jest calkowita to czy zostanie rzucony wyjatek -> wyswietlony komunikat o bledzie'''
        machine = Machine()
        with self.assertRaises(NotIntValueError):
            machine.checkValue("1.5")

    def test_should_return_NegativeNumberException(self):
        '''sprawdzenie czy jesli ilosc monet nie jest dodatnia to czy zostanie rzucony wyjatek -> wyswietlony komunikat o bledzie'''
        machine = Machine()
        with self.assertRaises(NegativeNumberValueError):
            machine.checkValue("-1")

    def test_should_correctly_return_price(self):
        '''sprawdzenie czy dodajac bilet, nastepnie wrzucajac monete, a nastepnie dodajac kolejny bilet cena jest odpowiednia'''
        machine = Machine()
        machine.calculateAllChosenTicketsPrice("20 min ulgowy", '1')
        machine.addMoneyToMachine('1')
        machine.calculateAllChosenTicketsPrice("20 min ulgowy", '1')
